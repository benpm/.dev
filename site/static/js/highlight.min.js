  ...
  var hljs = {
    ...
    highlightElement: function(element) {
      ...
      if (element.className) {
        var matches = element.className.match(/(?:^|\s)(python|javascript|java|c|cpp|rust|nim|go|ruby|php)(?:\s|$)/);
        if (matches) {
          language = matches[1];
        }
      }
    ...
    highlight: function(code, lang) {
      ...
      if (['javascript', 'java', 'c', 'cpp', 'rust', 'go', 'php'].indexOf(lang) !== -1) {
        patterns.push({ regex: /(\/\/[^\n]*)/g, className: 'hljs-comment' });
      ...
      var langKeywords = keywords[lang] || keywords.python;
      patterns.push({ 
      ...
      for (var marker in markers) {
        result = result.replace(new RegExp(marker.replace(/\\/g, '\\\\'), 'g'), markers[marker]);
      }
